...
RTSP_IP = os.environ["RTSP_IP"] if "RTSP_IP" in os.environ else 'x.x.x.x'
fame_rtsp_url = f"rtsp://admin:xxx@{RTSP_IP}:554/Streaming/Channels/201"

IPADDRESS = os.environ["IPADDRESS"] if "IPADDRESS" in os.environ else '0.0.0.0'
PORT = os.environ["PORT"] if "PORT" in os.environ else 5003
PORT = int(PORT)
